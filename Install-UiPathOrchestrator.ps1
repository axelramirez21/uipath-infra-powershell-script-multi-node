...
function Main {
    ...
    if($installParamsExist -and $CertificateExist){
        ...
        $msiProperties = @{ }
        $msiProperties += @{
            "PARAMETERS_FILE" = $nuGetStoragePath+"installParams.json";
            "SECONDARY_NODE"     = "1";
        }

        #Install Orchestrator as a Secondary Node
        ...
        #This will not affect anything as long as identityPublicURL is not empty
        ...
        Install-UiPathOrchestratorEnterprise -msiPath "$($tempDirectory)\UiPathOrchestrator.msi" -logPath "$($sLogPath)\Install-UiPathOrchestrator.log" -outputParamsFile $nuGetStoragePath"installParams.json" -msiFeatures $msiFeatures -msiProperties $msiProperties

    }

    ######################ISS BINDING AND POST INSTALLATION TASKS #############################################
    ...
    $identityPublicURLNoHTTP = $identityPublicURL.Replace("https://","")
    New-WebBinding -Name "UiPath*" -IPAddress "*" -Protocol https -Port 443 -HostHeader $identityPublicURLNoHTTP
    ...
    try {
        ...
        TestOrchestratorConnection -orchestratorURL "https://$identityPublicURL"
        TestOrchestratorConnection -orchestratorURL "http://$identityPublicURL"
    }
    catch {
    ...
    if ($nuGetStoragePath) {
        ...
        if ($orchestratorVersion -lt "19.4.1") {
            ...
            Write-Host "Changing Uipath.Orchestrator.dll.config Storage.Location key to $LBvalue"
            ...
            if($PrimaryNode)
            {
                ...
                Write-Host "Making a copy of Uipath Storage folder content to : $LBvalue"
...
function Install-UiPathOrchestratorEnterprise {
    ...
    if ($outputParamsFile) {
        $msiProperties["OUTPUT_PARAMETERS_FILE"] = $outputParamsFile;
    }
...
function Set-AppSettings {
    ...
    $ConfigBackup = "$path\UiPath.Orchestrator.dll.config.$(Get-Date -Format yyyyMMdd_hhmmsstt).backup"
    try { Copy-Item -Path "$path\UiPath.Orchestrator.dll.config" -Destination $ConfigBackup -Force -EA 1 } catch { throw }
    Write-Verbose "Backed up '$path\UiPath.Orchestrator.dll.config' to '$ConfigBackup'"
    Log-Write -LogPath $sLogFile -LineValue "Backed up '$path\UiPath.Orchestrator.dll.config' to '$ConfigBackup'"


    $webconfig = Join-Path $path "UiPath.Orchestrator.dll.config"
    [bool] $found = $false
